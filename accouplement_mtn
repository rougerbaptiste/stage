...
mal_size <- rnorm(pop_mal,mean=2.75,sd=0.2) #vecteur avec tailles des males (loi normale)
...
while(i <= pop_fem){#on regarde toute la pop
  ...
  while(fem_partner[i]==0){ #tant que la femelle n'a pas de partenaire
    ...
    mal_rand <- sample(mal_num,1) #on tire un male au hasard
    ...
    if(i2 > pop_mal) fem_partner[i] <- -1 #si plus de male, boucle suivante
    i2 <- i2+1
  }
  i <- i+1
}
